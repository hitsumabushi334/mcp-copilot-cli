...
describe('spawnCommand (non-interactive)', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    (spawnMocked as unknown as vi.Mock).mockReset();
  });
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it('passes command and args through and resolves on close', async () => {
    ...
    (spawnMocked as unknown as vi.Mock).mockImplementation((cmd: string, args: string[]) => {
    ...
    const p = spawnCommand('copilot', ['-p', 'hello', '--model', 'claude-sonnet-4'], 1000);
    vi.advanceTimersByTime(5);
    const res = await p;
    expect((spawnMocked as any).mock.calls[0][0]).toBe('copilot');
    expect((spawnMocked as any).mock.calls[0][1]).toEqual(['-p', 'hello', '--model', 'claude-sonnet-4']);
  ...
  it('returns timeout when execution exceeds timeoutMs', async () => {
    const child = new FakeChild();
    (spawnMocked as unknown as vi.Mock).mockImplementation(() => {
  ...
  it('maps ENOENT spawn error to not_installed', async () => {
    const child = new FakeChild();
    (spawnMocked as unknown as vi.Mock).mockImplementation(() => child as any);
